...
hist_day_load  = 5     # ARCHIV history from 23 March
#
def test_msg_lmb():
    #bp()
...
def tabs_layout(nmb_tab, tab_tabs):
    ...
    elif nmb_tab == 2:  # Req HIST_PACK
        tabs = [
                [sg.Input(key='-CALEND_FROM-', size=(16,1)),
                 sg.Button('-Date_FROM-', size=(12, 1),  pad=(0,0))],
                 #sg.CalendarButton('DATE from', target='-CALEND_FROM-', format='%d.%m.%Y', no_titlebar=False, begin_at_sunday_plus=1,)],
                [sg.Input(key='-CALEND_TO-', size=(16,1)),
                 sg.Button('-Date_TO-',   size=(12, 1),  pad=(0,0))],
                 #sg.CalendarButton('DATE to  ', target='-CALEND_TO-',   format='%d.%m.%Y', no_titlebar=False, begin_at_sunday_plus=1,)]
                [sg.T(' ')],
                [sg.Button('-Load HIST-', size=(14, 1))],
               ]
...
def event_MENU(_gl, wndw, ev, val):
    ...
    if ev == '-Date_FROM-':
        ...
        dt_str = sg.popup_get_date(location=(0,0), no_titlebar=False, begin_at_sunday_plus=1)
        sg.ChangeLookAndFeel('SystemDefault')
        if dt_str != None:
            dt_str = datetime.strptime(str(dt_str), '(%m, %d, %Y)')
            wndw.FindElement('-CALEND_FROM-').Update(dt_str)
    #----------------------------------------
    if ev == '-Date_TO-':
        ...
        if dt_str != None:
            dt_str = datetime.strptime(str(dt_str), '(%m, %d, %Y)')
            wndw.FindElement('-CALEND_TO-').Update(dt_str)
    #----------------------------------------
    if ev == '-Load HIST-':
        print('-Load HIST-')
        #if (val['-CALEND_FROM-'] != '') and (val['-CALEND_TO-'] != ''):
            #dtt_from      = datetime.strptime(val['-CALEND_FROM-'], "%Y-%m-%d %H:%M:%S")
            #dtt_from_sec  = int(dtt_from.replace(tzinfo=timezone.utc).timestamp())
            #dtt_to      = datetime.strptime(val['-CALEND_TO-'],   "%Y-%m-%d %H:%M:%S")
            #dtt_to_sec  = int(dtt_to.replace(tzinfo=timezone.utc).timestamp())
            #try:
                #if dtt_from_sec < dtt_to_sec:
                    #rep = _gl.db_ARCHV.read_tbl_between('hist_PACK', dtt_from_sec, dtt_to_sec)
                    #if rep[0] == 0:
                        #sg.popup(rep[1][0], '\n', rep[1][-1], '\n',   grab_anywhere=True)
                #else:
                    #err_lmb('Load HIST',
                        #s_lmb('Error ! Date FROM should less then TO\n and also can not be TODAY') +
                        #s_lmb(dtt_from) + s_lmb(dtt_to))
            #except Exception as ex:
                #err_lmb('Load HIST', s_lmb('Error ! ' + str(ex) + '\nLets choice another date'))

        if (val['-CALEND_FROM-'] != '') and (val['-CALEND_TO-'] != ''):
            rep = _gl.get_hist_pck_period(
                datetime.strptime(val['-CALEND_FROM-'], "%Y-%m-%d %H:%M:%S"),
                datetime.strptime(val['-CALEND_TO-'],   "%Y-%m-%d %H:%M:%S"))
...
def main():
    ...
    while True: #--- Menu & Tab Definition ----------------------------#
        tab_keys = ('-Tbl_HIST_FUT-', '-Tbl_HIST_PACK-', '-Req_HIST_PACK-', '-CFG_SOFT-')
        tab_tabs = ( 'Tab HIST_FUT',   'Tab HIST_PACK',   'Req HIST_PACK',   'Conf SOFT')
    ...
    while True: #--- Main Cycle ---------------------------------------#
        ...
        #
        if event in ['APPEND', 'CALC', 'Edit PACK CONF', 'Show PACK TABL',  'About...']:
            event_MENU(_gl, window, event, values)
        #
        # just for DEBAGGING
        if event in ['One PACK', '-Date_FROM-', '-Date_TO-', '-Load HIST-' ]:
            event_MENU(_gl, window, event, values)
        #
    window.close()
    return 0
